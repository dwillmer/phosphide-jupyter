...
for (name in _keycodes) {
    ...
    if (names.length === 1) {
        var n = names[0];
        exports.keycodes[n] = _keycodes[n];
        inv_keycodes[_keycodes[n]] = n;
    }
    else {
        var primary = names[0];
        var secondary = names[1];
        exports.keycodes[primary] = _keycodes[name];
        exports.keycodes[secondary] = _keycodes[name];
        inv_keycodes[_keycodes[name]] = primary;
    }
}
exports.normalize_key = function (key) {
    return inv_keycodes[exports.keycodes[key]];
};
exports.normalize_shortcut = function (shortcut) {
    /**
    ...
    if (shortcut.indexOf(',') !== -1) {
        ...
        sht = _.map(sht, exports.normalize_shortcut);
        return shortcut;
    }
    shortcut = shortcut.replace(/comma/g, ','); // catch shortcuts using '-' key
    var values = shortcut.split("-");
    if (values.length === 1) {
        return exports.normalize_key(values[0]);
    }
    else {
        var modifiers = values.slice(0, -1);
        var key = exports.normalize_key(values[values.length - 1]);
        modifiers.sort();
        return modifiers.join('-') + '-' + key;
    }
};
exports.shortcut_to_event = function (shortcut, type) {
    /**
     * Convert a shortcut (shift-r) to a jQuery Event object
     **/
    type = type || 'keydown';
    shortcut = exports.normalize_shortcut(shortcut);
    shortcut = shortcut.replace(/-$/, 'minus'); // catch shortcuts using '-' key
    var values = shortcut.split("-");
    var modifiers = values.slice(0, -1);
    var key = values[values.length - 1];
    var opts = { which: exports.keycodes[key] };
...
exports.only_modifier_event = function (event) {
    /**
...
exports.event_to_shortcut = function (event) {
    /**
...
exports.ShortcutManager = function (delay, events, actions, env) {
    /**
...
exports.ShortcutManager.prototype.clearsoon = function () {
    /**
...
exports.ShortcutManager.prototype.clearqueue = function () {
    /**
...
exports.ShortcutManager.prototype.get_action_shortcut = function (name) {
...
exports.ShortcutManager.prototype.help = function () {
    var help = [];
    var ftree = flatten_shorttree(this._shortcuts);
...
exports.ShortcutManager.prototype.clear_shortcuts = function () {
    this._shortcuts = {};
};
exports.ShortcutManager.prototype.get_shortcut = function (shortcut) {
    /**
...
exports.ShortcutManager.prototype._get_leaf = function (shortcut_array, tree) {
    /**
     * @private
...
exports.ShortcutManager.prototype.set_shortcut = function (shortcut, action_name) {
    if (typeof (action_name) !== 'string') {
        throw new Error('action is not a string' + String(action_name));
    }
    if (typeof (shortcut) === 'string') {
        shortcut = shortcut.split(',');
    }
...
exports.ShortcutManager.prototype._is_leaf = function (shortcut_array, tree) {
    if (shortcut_array.length === 1) {
...
exports.ShortcutManager.prototype._remove_leaf = function (shortcut_array, tree, allow_node) {
    if (shortcut_array.length === 1) {
...
exports.ShortcutManager.prototype._set_leaf = function (shortcut_array, action_name, tree) {
    var current_node = tree[shortcut_array[0]];
    if (shortcut_array.length === 1) {
...
exports.ShortcutManager.prototype.add_shortcut = function (shortcut, data, suppress_help_update) {
    /**
...
exports.ShortcutManager.prototype.add_shortcuts = function (data) {
    /**
...
exports.ShortcutManager.prototype.remove_shortcut = function (shortcut, suppress_help_update) {
    /**
...
exports.ShortcutManager.prototype.call_handler = function (event) {
    /**
     ...
     **/
    this.clearsoon();
    if (exports.only_modifier_event(event)) {
        return true;
    }
    var shortcut = exports.event_to_shortcut(event);
...
exports.ShortcutManager.prototype.handles = function (event) {
    var shortcut = exports.event_to_shortcut(event);
